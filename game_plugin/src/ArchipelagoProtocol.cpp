...
std::optional<json> ParseObject(const std::string_view message,
                                const std::string_view type) {
    auto parsed = json::parse(message, nullptr, false);
    if (parsed.is_discarded() || !parsed.is_object() ||
        parsed.value("type", std::string{}) != type) {
        return std::nullopt;
    }
    return parsed;
}
...
std::optional<std::vector<std::string>> ReadNames(const json& object,
                                                  const char* key) {
    if (!object.contains(key) || !object[key].is_array() ||
        object[key].size() > maximumListSize) {
        return std::nullopt;
    }
...
std::optional<ReceivedItemMessage> ParseReceivedItemMessage(
    ...
    if (!parsed || !parsed->contains("name") ||
...
std::string SerializeItemAcknowledgement(std::uint64_t index, bool accepted) {
    return json{{"type", "item_ack"}, {"index", index}, {"accepted", accepted}}
        .dump();
}
...
std::string SerializeSessionReject(const std::string_view reason,
                                   const std::string_view message) {
    return json{
        {"type", "session_rejected"}, {"reason", reason}, {"message", message}}
        .dump();
}
...
std::optional<SessionReadyMessage> ParseSessionReadyMessage(
    ...
    if (!parsed || !parsed->contains("seed_name") ||
        !(*parsed)["seed_name"].is_string()) {
        return std::nullopt;
    }
    ...
    if (!protocol || !team || !slot || seedName.empty() ||
        seedName.size() > maximumNameSize) {
        return std::nullopt;
    }
    const auto unlockables = ReadNames(*parsed, "managed_unlockables");
    const auto cheats = ReadNames(*parsed, "managed_cheats");

    if (!unlockables || !cheats || !parsed->contains("features") ||
        !(*parsed)["features"].is_object() ||
        !parsed->contains("enabled_progression") ||
        !(*parsed)["enabled_progression"].is_object()) {
        return std::nullopt;
    }
    ...
    const auto blockUnlockables =
        ReadBoolean(features, "block_vanilla_unlockables");
    ...
    if (!exclusiveRespect || !blockUnlockables || !notorietyTraps ||
        !missions || !activities || !hitman || !chopShop || !cds || !races) {
        return std::nullopt;
    }
...
std::string SerializeGameContext(const std::optional<std::uint32_t> checksum,
                                 std::uint64_t nextIndex, bool provisional,
                                 bool needsCursor) {
...
std::string SerializeSaveRevision(std::uint32_t checksum,
                                  std::uint64_t nextIndex) {
    return json{{"type", "save_revision"},
                {"checksum", checksum},
                {"next_index", nextIndex}}
        .dump();
}
