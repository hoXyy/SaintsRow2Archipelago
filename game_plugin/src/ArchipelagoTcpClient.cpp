...
ArchipelagoTcpClient::ArchipelagoTcpClient(std::uint16_t port,
...
void ArchipelagoTcpClient::BeginConnect() {
    ...
    if (const int result = connect(
            socket_, reinterpret_cast<sockaddr*>(&address), sizeof(address));
        result == 0) {
...
void ArchipelagoTcpClient::FlushOutgoing() {
    while (!outgoing_.empty()) {
        const int sent = send(socket_, outgoing_.data(),
                              static_cast<int>(outgoing_.size()), 0);
