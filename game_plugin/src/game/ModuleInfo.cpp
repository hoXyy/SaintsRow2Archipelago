...
namespace sr2ap {
namespace {
bool IsRangeInsideImage(std::uintptr_t base, std::size_t imageSize,
...
bool Matches(const ModuleInfo& module, std::uintptr_t rva,
             const char (&expected)[Size]) {
    if (!IsRangeInsideImage(module.base, module.imageSize, module.base + rva,
                            Size)) {
        return false;
    }
    std::array<char, Size> actual{};
    return SafeCopy(reinterpret_cast<const void*>(module.base + rva),
...
std::optional<ModuleInfo> InspectModule(HMODULE module) {
    ...
    if (!SafeCopy(reinterpret_cast<const void*>(base), &dos, sizeof(dos)) ||
    ...
    IMAGE_NT_HEADERS nt{};
    if (ntAddress < base ||
        !SafeCopy(reinterpret_cast<const void*>(ntAddress), &nt, sizeof(nt)) ||
    ...
    for (unsigned index = 0; index < nt.FileHeader.NumberOfSections; ++index) {
        ...
        if (!IsRangeInsideImage(base, result.imageSize,
                                base + section.VirtualAddress,
                                section.Misc.VirtualSize)) {
            continue;
        }
        result.sections.push_back({name, base + section.VirtualAddress,
                                   section.Misc.VirtualSize,
...
bool IsSupportedExecutable(const ModuleInfo& module) {
    ...
    static constexpr char patch12Pdb[]{
        "g:\\Projects\\SaintsRow\\sr2\\main\\code\\SR2___Win32_Final\\SR2_pc_"
        "final.pdb"};
    return Matches(module, 0x00120BA0, winMainPrologue) &&
           Matches(module, 0x00A66958, patch12Pdb);
}
}  // namespace sr2ap
