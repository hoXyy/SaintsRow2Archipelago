...
std::string SerializeProgressionStatus(const HitmanSnapshot& hitman,
    ...
    const auto retrieved =
        std::count_if(chopShop.vehicles.begin(), chopShop.vehicles.end(),
                      [](const auto& value) { return value.retrieved; });
    const auto missionsComplete =
        std::count_if(missions.missions.begin(), missions.missions.end(),
                      [](const auto& value) { return value.complete; });

    output << "game_ready="
           << (hitman.result == HitmanReadResult::Success ? 1 : 0) << '\n'
           << "reader_ready="
           << (hitman.result == HitmanReadResult::Success ? 1 : 0) << '\n'
    ...
    if (activities.result == ActivityReadResult::Success) {
        for (const auto& instance : activities.instances) {
            output << instance.instanceTag << '=' << instance.completedLevels
                   << '\n'
                   << instance.instanceTag << "_flags=0x" << std::uppercase
                   << std::hex << std::setw(2) << std::setfill('0')
                   << static_cast<unsigned>(instance.completionFlags)
                   << std::dec << std::setfill(' ') << '\n';
        }
    }

    const auto medalCount = std::count_if(
        racing.races.begin(), racing.races.end(), [](const auto& race) {
    ...
    if (cds.result == CdReadResult::Success) {
        const std::unordered_set<std::uint32_t> collected(
            cds.collectedIds.begin(), cds.collectedIds.end());
        ...
        std::sort(definitions.begin(), definitions.end(),
                  [](const auto* left, const auto* right) {
                      return std::string(left->districtKey) <
                             std::string(right->districtKey);
                  });
