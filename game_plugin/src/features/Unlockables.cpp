...
struct UnlockableController::Implementation {
    ...
    bool Install(bool blockVanilla,
        ...
        arrayPointerOperand =
            game->base + addresses::kUnlockableArrayPointerOperandRva;
    ...
    void Remove() {
        ...
        if (const auto disabled = processorHook.disable(); !disabled) {
            LogError("Unlockables",
                     "SafetyHook could not disable the processor hook");
            return;
        }
        installed = false;
        active.store(nullptr, std::memory_order_release);
    ...
    bool QueueReceivedItem(const std::string_view itemName) {
        ...
        SafeCopy(reinterpret_cast<const void*>(queueCountAddress), &queueBefore,
                 sizeof(queueBefore));
        reinterpret_cast<EnqueueFunction>(enqueueAddress)(
    ...
    static void __stdcall Hook(void* const item) {
        ...
        if (self->IsManagedHash(hash)) {
            if (self->ConsumeAllowance(hash)) {
                ...
                LogInfo("Unlockables", "Blocked vanilla unlockable hash=" +
                                           std::to_string(hash));
            }
            return;
        }
