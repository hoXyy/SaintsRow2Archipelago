...
class SessionRuntime {
   public:
    SessionRuntime(bool gameSupported,
                   std::filesystem::path revisionJournalPath)
    ...
    bool InstallSaveMonitoring(SaveRevisionMonitor& monitor, bool enabled) {
        saveMonitoringInstalled_ =
            ...
            monitor.Install(
                [this](std::uint32_t checksum) { OnSaveLoaded(checksum); },
    ...
    void Connect(std::uint16_t port) {
        client_.emplace(
            port, [this](std::string_view message) { HandleMessage(message); });
    }
    ...
    void PollNetwork() {
        ...
        if (!connected && networkWasConnected_) {
            ...
            LogInfo("Session",
                    "TCP disconnected; gameplay policy remains latched");
        }
        networkWasConnected_ = connected;
    }

    void UpdateReadiness(GameReadiness current) {
        if (current == GameReadiness::MainMenu &&
        ...
        } else if (IsGameplayLoaded(current) &&
                   deliveryState_ == DeliveryContextState::waitingForGameplay) {
            activeSaveChecksum_.reset();
            nextItemIndex_ = 0;
            deliveryState_ = DeliveryContextState::provisional;
            LogInfo("Items",
                    "Gameplay ready without save load; provisional cursor "
                    "initialized");
            SendGameContext();
        }
    }
    ...
    void SendProgression(const ProgressionEvent& event) {
        if (client_ && communicationsActive_ && sessionConfiguration_ &&
            ProgressionEnabled(event.kind)) {
   ...
   private:
    void SendGameContext() {
        ...
        client_->SendLine(SerializeGameContext(
            activeSaveChecksum_, nextItemIndex_,
            deliveryState_ == DeliveryContextState::provisional,
            deliveryState_ == DeliveryContextState::awaitingCursor));
    }

    RevisionSession CurrentRevisionSession() const {
        return {sessionConfiguration_->seedName, sessionConfiguration_->team,
                sessionConfiguration_->slot};
    }
    ...
    void SendPendingRevisions() {
        ...
        for (const auto& revision :
             revisionJournal_.Pending(CurrentRevisionSession())) {
            client_->SendLine(
                SerializeSaveRevision(revision.checksum, revision.nextIndex));
        }
    }
    ...
    void BeginRevisionSync() {
        ...
        if (pending.empty()) {
            ...
            LogInfo("SaveRevision",
                    "Synchronizing " + std::to_string(pending.size()) +
                        " durable revision(s) before item delivery");
        }
    }

    void HandleSaveRevisionAcknowledgement(
        const SaveRevisionAcknowledgementMessage& acknowledgement) {
        if (!sessionConfiguration_ || !acknowledgement.accepted) {
            LogError("SaveRevision",
                     "Client rejected revision checksum=" +
                         Hex(acknowledgement.checksum) + " next_index=" +
                         std::to_string(acknowledgement.nextIndex));
            return;
        }
        if (!revisionJournal_.Acknowledge(CurrentRevisionSession(),
                                          acknowledgement.checksum,
                                          acknowledgement.nextIndex)) {
            LogWarning("SaveRevision",
                       "Ignored stale or mismatched revision "
                       "acknowledgement checksum=" +
        ...
        LogInfo("SaveRevision",
                "Revision persisted by client checksum=" +
                    Hex(acknowledgement.checksum) +
                    " next_index=" + std::to_string(acknowledgement.nextIndex));
        if (revisionSyncPending_ &&
            revisionJournal_.Pending(CurrentRevisionSession()).empty()) {
            revisionSyncPending_ = false;
            SendGameContext();
        }
    }
    ...
    void OnSaveLoaded(std::uint32_t checksum) {
        ...
        deliveryState_ = DeliveryContextState::awaitingCursor;
        LogInfo("SaveRevision",
                "Loaded checksum=" + Hex(checksum) + "; awaiting AP cursor");
        SendGameContext();
    }
    ...
    void OnSaveWritten(std::uint32_t checksum) {
        if (!IsDeliveryReady(deliveryState_)) {
            LogWarning(
                "SaveRevision",
                "Generated save checksum before AP cursor was established");
            return;
        }
        activeSaveChecksum_ = checksum;
        ...
        if (!sessionConfiguration_ || !revisionJournalAvailable_) {
            LogWarning("SaveRevision",
                       "Could not associate generated checksum with an AP "
                       "session");
            return;
        }
        revisionJournal_.Record(CurrentRevisionSession(), checksum,
                                nextItemIndex_);
        if (!PersistRevisionJournal()) {
            return;
        }
        LogInfo("SaveRevision",
                "Generated checksum=" + Hex(checksum) +
    ...
    bool SameSession(const SessionReadyMessage& session) const {
        return sessionConfiguration_ &&
               session.seedName == sessionConfiguration_->seedName &&
               session.team == sessionConfiguration_->team &&
               session.slot == sessionConfiguration_->slot;
    }

    bool SupportsManagedItems(const SessionReadyMessage& session) const {
        return std::all_of(session.managedUnlockables.begin(),
                           session.managedUnlockables.end(),
                           [](const auto& name) {
                               return UnlockableController::SupportsItem(name);
                           }) &&
               std::all_of(session.managedCheats.begin(),
                           session.managedCheats.end(), [](const auto& name) {
    ...
    bool InstallPolicies(const SessionReadyMessage& session) {
        ...
        unlockablesInstalled_ =
            unlockablesRequested &&
        ...
        notoriety_.Remove();
        cheats_.Remove();
        cheatsInstalled_ = notorietyInstalled_ = respectInstalled_ =
            unlockablesInstalled_ = false;
        return false;
    }

    void HandleSession(const SessionReadyMessage& session) {
        if (session.protocol != 3 || !gameSupported_ ||
            !revisionJournalAvailable_) {
            LogWarning("Session",
                       "Rejected unsupported session protocol, executable, "
                       "or revision journal");
            return;
        }
        if (sessionLatched_) {
            if (!SameSession(session)) {
                LogWarning("Session",
                           "Rejected conflicting AP session; restart SR2 "
                           "to switch");

                if (client_) {
                    constexpr std::string_view message =
                        "Conflicting AP session detected by game "
                        "integration plugin, please restart Saints Row "
                        "2.";

                    client_->SendLine(
                        SerializeSessionReject("session_reject", message));
                }
        ...
        if (!InstallPolicies(session)) {
            LogError("Session",
                     "AP gameplay policy activation failed; session rejected");
            return;
        }
        ...
        communicationsActive_ = true;
        LogInfo("Session",
                "AP gameplay policy activated seed=" + session.seedName +
                    " team=" + std::to_string(session.team) +
                    " slot=" + std::to_string(session.slot));
        BeginRevisionSync();
    }
    ...
    void HandleSaveContext(const SaveContextMessage& context) {
        if (!communicationsActive_ || !saveMonitoringInstalled_ ||
            deliveryState_ != DeliveryContextState::awaitingCursor ||
            !activeSaveChecksum_ || context.checksum != *activeSaveChecksum_) {
            LogWarning("Items", "Rejected save context for inactive checksum=" +
                                    Hex(context.checksum));
            return;
        }
        nextItemIndex_ = context.nextIndex;
        deliveryState_ = DeliveryContextState::activeRevision;
        LogInfo("Items",
                "Save context ready checksum=" + Hex(context.checksum) +
                    " next_index=" + std::to_string(nextItemIndex_));
    }
    ...
    void HandleItem(const ReceivedItemMessage& item) {
        if (!communicationsActive_ || !IsDeliveryReady(deliveryState_) ||
            ...
            LogWarning("Items",
                       "Deferred item until gameplay is interactive index=" +
                           std::to_string(item.index));
            Acknowledge(item.index, false);
            return;
        }
        if (item.index < nextItemIndex_) {
            LogInfo("Items", "Acknowledged duplicate index=" +
                                 std::to_string(item.index));
        ...
        if (item.index != nextItemIndex_) {
            LogWarning(
                "Items",
                "Rejected out-of-order index=" + std::to_string(item.index) +
    ...
    DeliveryContextState deliveryState_{
        DeliveryContextState::waitingForGameplay};
    bool gameSupported_{};
