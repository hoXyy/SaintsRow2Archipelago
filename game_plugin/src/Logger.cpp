...
namespace sr2ap::log {
namespace {
[[nodiscard]] constexpr rust::LogLevel ToRustLevel(const Level level) noexcept {
    switch (level) {
        case Level::Trace:
...
void Write(const Level level, const std::string_view subsystem,
           const std::string_view message) {
    try {
        rust::log_write(ToRustLevel(level), Bytes(subsystem), Bytes(message));
    } catch (const ::rust::Error&) {
