...
TEST(ReaderResultTest, ConvertsEveryValueToItsProtocolName) {
    ...
    EXPECT_STREQ(ToString(ReaderResult::UnsupportedVersion),
                 "unsupported_version");
    EXPECT_STREQ(ToString(ReaderResult::GameNotReady), "game_not_ready");
    EXPECT_STREQ(ToString(ReaderResult::ManagerUnavailable),
                 "manager_unavailable");
    ...
    EXPECT_STREQ(ToString(ReaderResult::ReaderUnavailable),
                 "reader_unavailable");
}
}  // namespace sr2ap
