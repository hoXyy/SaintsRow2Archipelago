...
TEST(ProgressionEventTrackerTest,
     UnavailabilityInvalidatesWithoutFalseRecoveryEvent) {
    ProgressionEventTracker tracker;
    HitmanSnapshot snapshot{ReaderResult::Success, {{"target", 1, 1, false}}};
...
TEST(ProgressionEventTrackerTest,
     MissionsAndActivitiesEmitExistingProgressOnBaseline) {
    ProgressionEventTracker tracker;
    const auto missions = tracker.Observe(MissionSnapshot{
        ReaderResult::Success, {{"complete", true}, {"incomplete", false}}});
    ASSERT_EQ(missions.events.size(), 1U);
    EXPECT_EQ(missions.events.front().key, "complete");
    EXPECT_EQ(missions.events.front().current, 1U);

    const auto activities = tracker.Observe(
        ActivitySnapshot{ReaderResult::Success, {{"activity", 2, 3, 0x05}}});
...
TEST(ProgressionEventTrackerTest,
     CdsEmitOnlyAdditionsAndRepresentUnknownIdsSafely) {
    ProgressionEventTracker tracker;
    const auto known = GetCdDefinitions().front();
    EXPECT_TRUE(tracker.Observe(CdSnapshot{ReaderResult::Success, 50, {}})
                    .events.empty());
    const auto added =
        tracker.Observe(CdSnapshot{ReaderResult::Success, 50, {known.id, 1}});
