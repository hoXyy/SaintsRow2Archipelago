...
TEST(StatusSerializationTest, SerializesCountsFlagsAndSuccessfulEntries) {
    const HitmanSnapshot hitman{
        ReaderResult::Success, {{"hit_a", 1, 1, true}, {"hit_b", 1, 2, false}}};
    const ChopShopSnapshot chopShop{ReaderResult::Success,
                                    {{"car_a", 1, 1, true, 500, 10}}};
    const MissionSnapshot missions{ReaderResult::Success,
                                   {{"m01", true}, {"m02", false}}};
    const ActivitySnapshot activities{ReaderResult::Success,
                                      {{"act", 2, 3, 0x05}}};
