...
TEST(ArchipelagoProtocolTest, ParsesReceivedItemAndRejectsInvalidFields) {
    const auto item = ParseReceivedItemMessage(
        R"({"type":"item","index":17,"name":"Vehicle: Super Taxi"})");
    ASSERT_TRUE(item);
    EXPECT_EQ(item->index, 17U);
    EXPECT_EQ(item->name, "Vehicle: Super Taxi");
    EXPECT_FALSE(
        ParseReceivedItemMessage(R"({"type":"item","index":-1,"name":"x"})"));
    EXPECT_FALSE(
        ParseReceivedItemMessage(R"({"type":"item","index":1.5,"name":"x"})"));
...
TEST(ArchipelagoProtocolTest, ParsesSaveContextWithIntegerBoundaries) {
    const auto context = ParseSaveContextMessage(
        R"({"type":"save_context","checksum":4294967295,"next_index":18})");
    ASSERT_TRUE(context);
    EXPECT_EQ(context->checksum, 4294967295U);
    EXPECT_FALSE(ParseSaveContextMessage(
        R"({"type":"save_context","checksum":4294967296,"next_index":18})"));
    EXPECT_FALSE(ParseSaveContextMessage(
        R"({"type":"save_context","checksum":1,"next_index":-1})"));
}
...
TEST(ArchipelagoProtocolTest, ParsesSaveRevisionAcknowledgement) {
    ...
    EXPECT_FALSE(ParseSaveRevisionAcknowledgementMessage(
        R"({"type":"save_revision_ack","checksum":1,"next_index":18})"));
}

TEST(ArchipelagoProtocolTest,
     ParsesCompleteSessionAndDeduplicatesManagedNames) {
...
TEST(ArchipelagoProtocolTest, RejectsIncompleteOrOversizedSessions) {
    EXPECT_FALSE(ParseSessionReadyMessage(
        R"({"type":"session_ready","protocol":3,"seed_name":"x","team":0,"slot":1})"));
    std::string longName(129, 'x');
    EXPECT_FALSE(ParseSessionReadyMessage(
        "{\"type\":\"session_ready\",\"protocol\":2,\"seed_name\":\"" +
        longName + "\",\"team\":0,\"slot\":1}"));
}

TEST(ArchipelagoProtocolTest, SerializesEveryProgressionCategory) {
    const std::pair<ProgressionKind, const char*> cases[]{
        {ProgressionKind::Hitman, "hitman"},
...
TEST(ArchipelagoProtocolTest, SerializesContextAndAcknowledgements) {
    EXPECT_EQ(SerializeItemAcknowledgement(17, true),
              R"({"accepted":true,"index":17,"type":"item_ack"})");
    EXPECT_EQ(
        SerializeGameContext(std::nullopt, 4, true, false),
        R"({"checksum":null,"needs_cursor":false,"next_index":4,"provisional":true,"type":"game_context"})");
    EXPECT_EQ(SerializeSaveRevision(5, 18),
              R"({"checksum":5,"next_index":18,"type":"save_revision"})");
