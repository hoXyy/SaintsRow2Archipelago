...
[[nodiscard]] std::string SerializeItemAcknowledgement(std::uint64_t index,
                                                       bool accepted);
[[nodiscard]] std::optional<SaveContextMessage> ParseSaveContextMessage(
    std::string_view message);
[[nodiscard]] std::optional<SaveRevisionAcknowledgementMessage>
ParseSaveRevisionAcknowledgementMessage(std::string_view message);
[[nodiscard]] std::optional<SessionReadyMessage> ParseSessionReadyMessage(
    std::string_view message);
[[nodiscard]] bool IsSessionEndMessage(std::string_view message);
[[nodiscard]] std::string SerializeGameContext(
    std::optional<std::uint32_t> checksum, std::uint64_t nextIndex,
    bool provisional, bool needsCursor);
[[nodiscard]] std::string SerializeSaveRevision(std::uint32_t checksum,
                                                std::uint64_t nextIndex);
[[nodiscard]] std::string SerializeSessionReject(
    const std::string_view reason, const std::string_view message);
}  // namespace sr2ap
